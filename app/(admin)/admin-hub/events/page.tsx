    ...
    <>
        ...
        <div style={{ display: "flex", gap: ".35rem", flexWrap: "wrap" }}>
          {EVENT_TYPES.map((t) => (
            <button key={t} onClick={() => setTypeFilter(t)}
              style={{ padding: ".3rem .75rem", borderRadius: 0, fontSize: ".72rem", fontFamily: "var(--fs)", fontWeight: 600, cursor: "pointer", border: "1px solid", borderColor: typeFilter === t ? "var(--green)" : "var(--border)", background: typeFilter === t ? "var(--green)" : "#fff", color: typeFilter === t ? "#fff" : "var(--muted-foreground)", transition: "all .15s" }}>
              {t}
            </button>
          ))}
        </div>
      ...
      {!loading && filtered.length > 0 && (
        <div style={{ display: "flex", flexDirection: "column", gap: ".6rem" }}>
          {filtered.map((event) => (
            <div key={event.id} style={{ display: "flex", gap: "1rem", alignItems: "flex-start", padding: ".9rem 1rem", background: "var(--card)", border: `1px solid ${event.is_approved ? "var(--border)" : "#FECACA"}`, borderRadius: 0, opacity: event.is_approved ? 1 : 0.65 }}>
              {/* Color dot */}
              <div style={{ flexShrink: 0, width: 10, height: 10, borderRadius: 0, background: TYPE_COLORS[event.event_type] || "#059669", marginTop: 6 }} />

              {/* Content */}
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: "flex", alignItems: "center", gap: ".5rem", flexWrap: "wrap", marginBottom: ".25rem" }}>
                  <span style={{ fontSize: ".65rem", padding: ".15rem .45rem", background: TYPE_COLORS[event.event_type] || "#059669", color: "#fff", borderRadius: 0, fontFamily: "var(--fm)", fontWeight: 600 }}>{event.event_type}</span>
                  <span style={{ fontSize: ".65rem", padding: ".15rem .45rem", background: event.format === "Online" ? "#EFF6FF" : "#ECFDF5", color: event.format === "Online" ? "#1D4ED8" : "#065F46", borderRadius: 0, fontFamily: "var(--fm)", display: "flex", alignItems: "center", gap: ".2rem" }}>
                    {event.format === "Online" ? <Monitor size={10} /> : <Users size={10} />}{event.format}
                  </span>
                  <span style={{ fontSize: ".65rem", padding: ".15rem .45rem", background: "var(--cd)", color: "var(--muted-foreground)", borderRadius: 0, fontFamily: "var(--fm)", border: "1px solid var(--border)" }}>{event.topic}</span>
                  {!event.is_approved && <span style={{ fontSize: ".6rem", padding: ".1rem .4rem", background: "#FEF2F2", color: "#991B1B", borderRadius: 0, fontFamily: "var(--fm)" }}>Hidden</span>}
                </div>
              ...
              <div style={{ display: "flex", gap: ".4rem", flexShrink: 0, alignItems: "center" }}>
                <a href={event.url} target="_blank" rel="noopener noreferrer" title="Open" style={{ padding: ".35rem", borderRadius: 0, border: "1px solid var(--border)", background: "var(--card)", color: "var(--muted-foreground)", display: "flex", alignItems: "center", textDecoration: "none" }}>
                  <ExternalLink size={14} />
                </a>
                <button onClick={() => setEditEvent(event)} title="Edit" style={{ padding: ".35rem", borderRadius: 0, border: "1px solid var(--border)", background: "var(--card)", color: "var(--muted-foreground)", cursor: "pointer", display: "flex", alignItems: "center" }}>
                  <Pencil size={14} />
                </button>
                <button onClick={() => toggleApprove(event)} title={event.is_approved ? "Hide" : "Show"}
                  style={{ padding: ".35rem", borderRadius: 0, cursor: "pointer", display: "flex", alignItems: "center", border: "1px solid", borderColor: event.is_approved ? "#A7F3D0" : "#FECACA", background: event.is_approved ? "#ECFDF5" : "#FEF2F2", color: event.is_approved ? "#065F46" : "#991B1B" }}>
                  {event.is_approved ? <Check size={14} /> : <X size={14} />}
                </button>
                <button onClick={() => deleteEvent(event.id)} title="Delete" style={{ padding: ".35rem", borderRadius: 0, border: "1px solid #FECACA", background: "#FEF2F2", color: "#991B1B", cursor: "pointer", display: "flex", alignItems: "center" }}>
                  <Trash2 size={14} />
                </button>
              </div>
            </div>

...
- REJECT vague roundup/listicle posts covering many unrelated events at once (e.g. "Climate events 2026: Conferences, summits and more", "What's Up in GENeva | 13-19 April") — approve only a post about ONE specific, named event
- REJECT generic organization/company profile pages or personal bios that are not themselves an announcement of one specific event
- APPROVE only genuine upcoming event announcements with a clear title naming one specific event
...
5. **event_date**: Exact date as "YYYY-MM-DD" if explicitly stated in the title, excerpt, or the "raw_date" field (a date scraped directly from the page — prefer it over guessing from text, but note it may be an article publish date rather than the event date, so use judgment). Return null if no real event date can be determined — DO NOT GUESS.

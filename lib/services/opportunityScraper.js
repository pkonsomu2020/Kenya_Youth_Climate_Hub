...
const { OPPORTUNITY_SITES, OPPORTUNITY_SEARCH_QUERIES } = require("../config/opportunitySites");
...
// Generic fallback selectors for locating a deadline/date within a scraped
// list item. Sites rarely expose a dedicated "date" selector in our config,
// so we try the common patterns used across job-board/grant-listing markup.
...
async function fetchScrapfly(feedConfig) {
  try {
    ...
    $(selectors.list).slice(0, 15).each((i, el) => {
      ...
      const rawDate = selectors.date ? $(el).find(selectors.date).first().text().trim() : extractRawDate($, el);
  ...
  try {
    ...
    return (feed.items || []).slice(0, 15).map((item) => {
      ...
      return {
        ...
        // NOTE: this is the RSS article's publish date, not the actual
        // application deadline — it's a weak hint only, passed to the AI as context.
...
async function scrapeAllSites() {
  ...
  const googleItems = await fetchAllGoogleSearch(OPPORTUNITY_SEARCH_QUERIES);
